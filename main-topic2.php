...
// SQL作成&実行
$pdo = connect_to_db();
$sql = "SELECT * FROM contents_box LEFT OUTER JOIN (SELECT post_id, COUNT(id) AS like_count FROM like_table GROUP BY post_id) AS result_table ON contents_box.id = result_table.post_id WHERE tag='pacific'ORDER BY created_at DESC";
